            ...
            <div className="flex justify-between items-center">
                 <div className="border-b border-gray-700">
                    <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                        <button onClick={() => setActiveTab('payable')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'payable' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
                            Contas a Pagar
                        </button>
                        <button onClick={() => setActiveTab('receivable')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'receivable' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
                            Contas a Receber
                        </button>
                    </nav>
                </div>
...
const PayablesTable: React.FC = () => (
    <table className="w-full text-sm text-left text-gray-300">
        ...
        <tbody>
            {mockPayables.map(p => (
                <tr key={p.id} className="border-b border-gray-700 hover:bg-gray-600">
     ...
     <table className="w-full text-sm text-left text-gray-300">
        ...
        <tbody>
            {mockReceivables.map(r => (
                <tr key={r.id} className="border-b border-gray-700 hover:bg-gray-600">
